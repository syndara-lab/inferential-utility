...
class synthpopPlugin(Plugin):
    ...
    Args:
        ...
        visit_sequence: list[int].
            A character vector of names of variables or an integer vector of their column indices specifying the order of synthesis. 
    ...
    def _fit(self, X: DataLoader, *args: Any, **kwargs: Any) -> 'synthpopPlugin':
        ...
        # visit sequence 
        if not bool(set(self.visit_sequence)): # if visit_sequence is not specificied
            visit_sequence = range(df.shape[1])
        else:
            visit_sequence = self.visit_sequence
        r_visit_sequence = ro.IntVector(np.array(visit_sequence)+1) # add 1 to convert Python index to R index
        ...
        self.params = {'data': r_df,
                       'method': 'parametric',
                       'visit.sequence': r_visit_sequence,
    ...
    def _generate(self, count: int, syn_schema: Schema, **kwargs: Any) -> DataLoader:
        ...
        self.params['seed'] = ro.IntVector([self.params['seed']])
